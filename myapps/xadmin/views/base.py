...
class ModelAdminView(CommAdminView):
    fields = None  # 列表或元组类型，它定义将显示的字段
    exclude = None  # 列表或元组类型，它定义将从编辑页面中排除的字段
    ordering = None  # 它是一个dict类型，是reterieved Model queryset的默认顺序
    model = None  # 一个有界的Model类，在注册Model类时会自动分配给OptionClass
    remove_permissions = []
