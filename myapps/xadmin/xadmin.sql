BEGIN;
--
-- Create model Bookmark   python manage.py sqlmigrate xadmin 0001_initial
--
...
-- Create model Log
--
CREATE TABLE `xadmin_log` (`id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY, `action_time` datetime NOT NULL, `ip_addr` char(39) NULL, `object_id` longtext NULL, `object_repr` varchar(200) NOT NULL, `action_flag` varchar(32) NOT NULL, `message` longtext NOT NULL, `content_type_id` integer NULL, `user_id` integer NOT NULL);
--
...
ALTER TABLE `xadmin_bookmark` ADD CONSTRAINT `xadmin_bookmark_user_id_42d307fc_fk_auth_user_id` FOREIGN KEY (`user_id`) REFERENCES `auth_user` (`id`);
...
ALTER TABLE `xadmin_log` ADD CONSTRAINT `xadmin_log_user_id_bb16a176_fk_auth_user_id` FOREIGN KEY (`user_id`) REFERENCES `auth_user` (`id`);
ALTER TABLE `xadmin_usersettings` ADD CONSTRAINT `xadmin_usersettings_user_id_edeabe4a_fk_auth_user_id` FOREIGN KEY (`user_id`) REFERENCES `auth_user` (`id`);
ALTER TABLE `xadmin_userwidget` ADD CONSTRAINT `xadmin_userwidget_user_id_c159233a_fk_auth_user_id` FOREIGN KEY (`user_id`) REFERENCES `auth_user` (`id`);
COMMIT;
